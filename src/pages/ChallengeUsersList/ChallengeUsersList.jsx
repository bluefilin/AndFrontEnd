...
const ChallengeUsersList = () => {
  ...
  useEffect(() => {
    const listadoTodasRetos = async () => {
      ...
      const list = await RetosInnovaServices();
  ...
  const filterRetosEstado = (event) => {
    ...
    if (criterio != "todos") {
      const retosFiltered = retosList.filter((reto) => reto.estado?.nombre == criterio);
